...
impl LicensesArgs {
    pub async fn run(
        ...
        let results = self.collect_licenses(config, dir, lockfile_dir, &lockfile, recursive).await?;
        self.render_report(&results)
    }

    async fn collect_licenses(
        &self,
        config: &Config,
        dir: &std::path::Path,
        ...
        lockfile: &Lockfile,
        recursive: bool,
    ) -> miette::Result<IndexMap<String, BTreeMap<String, LicenseInfo>>> {
        ...
        let modules_dir = lockfile_dir.join(&config.modules_dir);
        let manifest = hoisting::read_manifest(&modules_dir);
        let hoisting_ctx = HoistingContext::new(config, lockfile_dir, dir, manifest.as_ref());

        Ok(group_by_license(&layout, dependencies, &hoisting_ctx).await)
    }
    ...
    fn render_report(
        ...
        if self.json {
            println!("{}", render::render_licenses_json(results_by_license)?);
            return Ok(());
        }
        if !results_by_license.is_empty() {
            render::print_license_table(results_by_license, self.long);
        }
        Ok(())
    }
}
...
async fn group_by_license(
    ...
    for (key, kind, name, version) in dependencies {
        let pkg_dir = hoisting::resolve_package_dir(&key, &name, layout, hoisting);
        record_package_license(&mut results_by_license, &pkg_dir, &name, version, kind).await;
    }
    results_by_license
}

async fn record_package_license(
    results: &mut IndexMap<String, BTreeMap<String, LicenseInfo>>,
    ...
    let license_group = results.entry(details.license.clone()).or_default();
