...
use pnpm_package_manifest::{is_truthy, safe_read_project_manifest_from_dir};
...
fn check_bin_crlf(bins: &[PathBuf], dir: &Path) -> Result<(), PackError> {
    for bin in bins {
        ...
        if has_shebang_with_crlf(&buffer[..n]) {
            let relative_path = bin
                .strip_prefix(dir)
                .unwrap_or(bin)
                .to_string_lossy()
                .replace('\\', "/");
            return Err(PackError::BinCrlf { path: relative_path });
        }
    }
    Ok(())
}
