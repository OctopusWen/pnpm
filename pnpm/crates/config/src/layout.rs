use super::{
    ...
    get_branch_candidates_from_git, get_current_branch, store_path,
};
...
impl Config {
    ...
    pub fn apply_git_branch_lockfile_derivation<Sys: GetCurrentDir>(&mut self) {
        ...
        let branch = get_current_branch::<GitHost>(&cwd)
            .or_else(|| Self::match_git_branch_lockfile::<GitHost>(&cwd));
    ...
    fn match_git_branch_lockfile<Sys: pnpm_git_utils::RunCommand>(cwd: &Path) -> Option<String> {
        let existing_files = Lockfile::git_branch_lockfiles(cwd).ok()?;
