...
invalidWorkspaceManifests.forEach((filename) => {
  test('incorrect workspace manifest ' + filename, async () => {
    ...
    writeYamlFileSync(filename, { packages: ['**', '!store/**'] })
...
test('recursive install with link-workspace-packages and shared-workspace-lockfile', async () => {
  ...
  const sharedLockfile = readYamlFileSync<LockfileFile>(WANTED_LOCKFILE)
...
test('shared-workspace-lockfile: install dependencies in projects that are relative to the workspace directory', async () => {
  ...
  writeYamlFileSync('monorepo/workspace/pnpm-workspace.yaml', { packages: ['../**', '!store/**'] })
...
test('shared-workspace-lockfile config is ignored if no pnpm-workspace.yaml is found', async () => {
  ...
  fs.writeFileSync('pnpm-workspace.yaml', 'sharedWorkspaceLockfile: true', 'utf8')
...
test('custom virtual store directory in a workspace with not shared lockfile', async () => {
  ...
  rimrafSync('project-1/virtual-store')
...
test('pnpm install --frozen-lockfile fails when lockfile contains an importer that was removed from workspace', async () => {
  ...
  await expect(
    execPnpm(['install', '--frozen-lockfile'])
  ).rejects.toThrow(/contains removed project/)
})
